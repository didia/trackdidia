...
`CLAUDE.md` must stay a byte-identical copy of this file. If you edit
`AGENTS.md`, update `CLAUDE.md` to match (CI enforces `cmp AGENTS.md CLAUDE.md`).
...
Prefer `npm run verify` (lint + typecheck + test + build) or `npm run verify:all`
(also Rust check/clippy and AGENTS/CLAUDE sync) when touching shared tooling or CI.
Pull requests run the same gates via `.github/workflows/ci.yml`.
...
There is currently no separate repository format-only gate beyond Biome
(`npm run lint` / `npm run format`); do not claim a legacy ESLint/Prettier script ran.

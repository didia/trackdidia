...
- Live Graph adapter: inbox delta baseline snapshot (`baselineAt`), paginated `@odata.nextLink`,
  post-baseline filtering, delta-link storage only after complete snapshot, immediate delta replay,
  invalid-delta reseed, message-level Outlook categories, and immutable IDs via
